  ...
  if (NexT.isFallback) {
    if (document.querySelectorAll) {
      ...
      NexT.boot.refreshHandle.push(() => {
        console.info("hexo-next-fallback: Highlight figure is not registered, fallback to re-register it.");
        try {
          let figures = NexT.fallback.findDescendants(document.documentElement, x => /(^|\s)highlight(\s|$)/.test(x.className));
          if (!figures.length) {
            figures = NexT.fallback.findDescendants(document.documentElement, x => x.tagName === "PRE");
          }
          for (let i = 0; i < figures.length; i++) {
            const element = figures[i];
            // Skip pre > .mermaid for folding and copy button
            if (NexT.fallback.findDescendant(element, x => /(^|\s)mermaid(\s|$)/.test(x.className))) { return; }
            let spans = [];
            const codes = NexT.fallback.findDescendants(element, x => /(^|\s)code(\s|$)/.test(x.className));
            for (let j = 0; j < codes.length; j++) {
              const code = codes[j];
              const lines = NexT.fallback.findDescendants(code, x => /(^|\s)line(\s|$)/.test(x.className));
              for (let k = 0; k < lines.length; k++) {
                const line = lines[k];
                const span = NexT.fallback.findDescendants(line, x => x.tagName === "SPAN");
            ...
            if (!spans.length) {
              // Hljs without line_number and wrap
              const highlights = NexT.fallback.findDescendants(element, x => x.tagName === "CODE" && /(^|\s)highlight(\s|$)/.test(x.className));
              for (let j = 0; j < highlights.length; j++) {
                spans = NexT.fallback.findDescendants(highlights[j], x => x.tagName === "SPAN");
