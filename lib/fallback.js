const init = () => {
  try {
    ...
    if (!window.CONFIG && document.querySelector) {
...
const search = () => {
  try {
    if (!window.LocalSearch && document.querySelector) {
      const search = document.querySelector(".menu-item-search .popup-trigger");
...
const sidebar = () => {
  try {
    ...
    if (!NexT.boot.motion) {
      addStyle("@media all and (min-width:992px){.sidebar-inner{visibility: inherit}}");
    }
    const utils = NexT.utils;
    if (utils.isFallback || !utils.activateSidebarPanel) {
      addStyle(".sidebar-inner{display:block}");
